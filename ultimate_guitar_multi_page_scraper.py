...
MAX_PAGES = 100
# 봇 감지 텍스트
BOT_DETECTION_TEXT = "Oops! We couldn't find that page."
...
def cleanup_files():
    """불필요한 파일을 정리합니다."""
    # 스크린샷 파일 삭제
    screenshot_files = glob.glob(os.path.join(OUTPUT_DIR, '*_screenshot.png'))
    for file in screenshot_files:
        try:
            os.remove(file)
            log_message(f"스크린샷 파일 삭제: {file}")
        except Exception as e:
            log_message(f"파일 삭제 중 오류 발생: {str(e)}")
    
    # HTML 소스 파일 삭제
    source_files = glob.glob(os.path.join(OUTPUT_DIR, '*_source.html'))
    for file in source_files:
        try:
            os.remove(file)
            log_message(f"소스 파일 삭제: {file}")
        except Exception as e:
            log_message(f"파일 삭제 중 오류 발생: {str(e)}")
    
    log_message("불필요한 파일 정리 완료")

async def check_bot_detection(page):
    """봇 감지 여부를 확인합니다."""
    try:
        page_text = await page.evaluate("document.body.innerText")
        if BOT_DETECTION_TEXT in page_text:
            log_message("봇 감지 텍스트가 발견되었습니다. 스크래핑을 중지합니다.")
            return True
        return False
    except Exception as e:
        log_message(f"봇 감지 확인 중 오류 발생: {str(e)}")
        return False
...
async def find_song_links_by_color(page):
    ...
    try:
        ...
        # 노래 제목 링크를 찾기 위한 여러 선택자 시도
        selectors = [
            'a[class*="qNp1Q SGCxQ"]',    # 지정된 클래스를 가진 링크
            'a[class="qNp1Q SGCxQ"]',      # 정확한 클래스를 가진 링크
            'a[href*="/tab/"]',            # /tab/ 경로를 가진 링크
            'a[href*="/tabs/"]',           # /tabs/ 경로를 가진 링크
            'a[class*="title"]',           # title 클래스를 가진 링크
        ...
        for selector in selectors:
            ...
            if elements:
                for element in elements:
                    ...
                    # 부모 요소에서 아티스트 정보 찾기
                    try:
                        # 상위 요소로 이동하면서 아티스트 정보 찾기
                        artist_element = await page.evaluate("""
                            (element) => {
                                // 현재 요소의 부모 요소들을 순회
                                let current = element;
                                while (current && current.parentElement) {
                                    // 아티스트 정보를 포함할 수 있는 요소 찾기
                                    const artistLink = current.parentElement.querySelector('a[href*="/artist/"]');
                                    if (artistLink) {
            ...
            song_links_data = await page.evaluate("""
                () => {
                    ...
                    const songLinks = allLinks.filter(link => {
                        ...
                        // 링크 URL이 /tab/를 포함하거나 특정 클래스를 가진 경우
                        return (href && (href.includes('/tab/') || href.includes('/tabs/'))) || 
                               (classes && (classes.includes('qNp1Q') || classes.includes('SGCxQ')));
                    });
...
async def main_async():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            while current_page <= MAX_PAGES and not bot_detected:
                ...
                if success:
                    ...
                    if await check_bot_detection(page):
                        bot_detected = True
                        log_message("봇 감지로 인해 스크래핑을 중지합니다.")
                    else:
        ...
        # 스크래핑이 정상적으로 완료되었고 봇 감지가 없었다면 불필요한 파일 정리
        if not bot_detected and current_page > start_page:
            log_message("스크래핑이 정상적으로 완료되었습니다. 불필요한 파일을 정리합니다.")
            cleanup_files()
